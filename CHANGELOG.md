...
- **Frontend assets now cache-bust on file change, not only on version bump.** `pforms-frontend` CSS/JS are versioned `PForms_VERSION.{filemtime}` — replacing plugin files without a version bump (hotfixes, staged rollouts) previously left asset URLs identical, so browsers/page caches/CDNs served stale files indefinitely (repeatedly surfaced as "the fix didn't deploy" during the 2026-07-10 iteration loop). `filemtime()` failure degrades to a stable `.0` suffix (version-only behavior, never an error). (`form-runtime-engine.php`.)
