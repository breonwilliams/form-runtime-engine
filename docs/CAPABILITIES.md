...
**`pforms_manage_forms`** — Controls access to the Form Entries admin pages
...
Constant in code: `PForms_Capabilities::MANAGE_FORMS`.
...
```bash
wp cap add editor pforms_manage_forms
wp cap add shop_manager pforms_manage_forms
```
...
if ( $role ) {
    $role->add_cap( 'pforms_manage_forms' );
}
```
...
calls `PForms_Capabilities::revoke_all_capabilities()`, which iterates every
...
|---|---|---|---|
| Form Runtime Engine | `pforms_manage_forms` | `PForms_Capabilities::MANAGE_FORMS` | `administrator` |
