...
class PForms_Connector_API {
    ...
    public static function get_connector_rulebook( $schema_reference_url ) {
        return array(
            ...
            'critical_rules' => array(
                ...
                'entry_read_gate' => 'formengine_list_entries and formengine_get_entry require the site administrator to have explicitly enabled entry-read access on the Form Entries → Connector admin page. Without it, these endpoints return 403 entry_access_disabled. Preflight reports the current state in `entry_read_enabled`.',
            ...
            'settings_hints' => array(
                ...
                'webhook'           => 'webhook_enabled, webhook_url and webhook_preset are TOP-LEVEL fields of formengine_create_form / formengine_update_form — NOT keys inside config.settings, where they are ignored (the dispatcher reads only the saved form record, so a form registered in PHP cannot have a webhook). Use HTTPS endpoints; plain http is accepted but sends entries unencrypted. The signing secret is admin-only and NOT exposed via this API. webhook_preset = "google_sheets" | "zapier" | "make" | "custom" — drives the smart default for option-label resolution: google_sheets resolves select/radio/checkbox option values to their human-readable labels in the payload, while zapier/make/custom emit raw values (typically machine-readable integrations prefer stable identifiers). settings.webhook_resolve_option_labels (boolean, optional) explicitly overrides the preset default — true forces labels regardless, false forces raw values regardless. Webhook payload includes a files array; each file row has field_key, file_name, file_size, mime_type, and file_url for downstream automations to fetch (Zapier → Drive, Make → S3, etc.). Webhook fires on the pforms_submission_complete action — AFTER files are attached — so file_url is always populated.',
                'notifications'     => 'settings.notification is an object {enabled, to, subject, from_name, from_email, reply_to}. Defaults: enabled=true, to={admin_email}, subject="New Form Submission", from_name={site_name}, from_email={admin_email}. System variables {admin_email}, {site_name}, {site_url}, {form_title} work in every one of these strings. {field:key} (substitutes the submitted value of the field whose key is "key" — option values are resolved to their labels) works in to, subject, from_name and reply_to, but NOT in from_email: the sender must be a fixed address on a domain the site\'s mail provider has verified, so a {field:...} token in from_email is rejected when the form is saved (and ignored at send time on forms stored before 1.10.0). reply_to has no default — when it is empty no Reply-To header is sent. To let the recipient reply straight to the submitter, set reply_to to "{field:<key of the form\'s email field>}", e.g. "{field:email}". settings.hide_empty_fields (boolean, default true) controls whether empty optional fields are skipped in the email body — set false to render every field with an em-dash placeholder for empty values, useful when emails feed downstream tooling that expects a fixed table shape. Conditionally-hidden fields (those with a conditions block that evaluates false) are ALWAYS skipped regardless of this flag.',
                'spam_protection'   => 'settings.spam_protection.honeypot and .timing_check default to true. settings.spam_protection.min_submission_time defaults to 3 seconds — a browser submission sooner than that is refused with "Please wait a moment before submitting." (the honeypot is the silent one: it shows the normal success message and stores nothing). The same content sent again within 60 seconds shows the success message only when the first copy was actually received; while the first copy is still being processed the visitor is asked to wait and try again. Rate limit default: 5 submissions per 3600 seconds (1 hour) per IP. The honeypot field name is dynamically generated per form (see critical_rules.honeypot_field_name_dynamic).',
     ...
     *
     * @param WP_REST_Request $request REST request (unused).
     * @return void Always writes a Markdown body: the knowledge map when it ships, else the generated rulebook.
     */
    public function handle_schema_document( $request ) {
        ...
        // The release ZIP leaves FRE_KNOWLEDGE_MAP.md out (a wordpress.org
        // compliance exclusion — see bin/build-release.sh), so on a site
        // installed from the ZIP the file above does not exist. Until
        // 2026-09-19 this route then answered 404, and the preflight still
        // told every assistant to "ALWAYS WebFetch" it. Serve the rulebook
        // the preflight is built from instead, with the form JSON Schema that
        // does ship — generated, never stale, and substantive enough that an
        // assistant is not left without rules.
        $generated = self::generated_schema_document();
        nocache_headers();
        status_header( 200 );
        header( 'Content-Type: text/markdown; charset=utf-8' );
        header( 'X-Content-Type-Options: nosniff' );
        echo $generated; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Markdown served raw by design; built from plugin-owned strings.
        exit;
    }

    /**
     * The connector rulebook as Markdown, for installs without the full
     * knowledge map: every rule the preflight returns, grouped, and the form
     * JSON Schema (docs/form-schema.json, which the release ships).
     *
     * @return string
     */
    public static function generated_schema_document() {
        $rulebook = self::get_connector_rulebook( '' );
        unset( $rulebook['read_first'] );

        $out   = array();
        $out[] = '# Promptless Forms connector rulebook';
        $out[] = '';
        $out[] = 'Generated by the plugin: the full knowledge map is not part of this install, so these are the rules the connector preflight is built from, followed by the form JSON Schema. They are the same rules, not a summary of different ones.';

        foreach ( $rulebook as $section => $rules ) {
            if ( ! is_array( $rules ) ) {
                continue;
            }
            $out[] = '';
            $out[] = '## ' . ucfirst( str_replace( '_', ' ', (string) $section ) );
            $out[] = '';
            foreach ( $rules as $key => $rule ) {
                $text  = is_string( $rule ) ? $rule : (string) wp_json_encode( $rule );
                $out[] = '- **' . $key . '** — ' . $text;
            }
        }

        $schema_path = ( defined( 'PForms_PLUGIN_DIR' ) ? PForms_PLUGIN_DIR : dirname( __DIR__, 2 ) . '/' ) . 'docs/form-schema.json';
        if ( is_readable( $schema_path ) ) {
            $schema = file_get_contents( $schema_path ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- local plugin file.
            if ( false !== $schema ) {
                $out[] = '';
                $out[] = '## Form JSON Schema';
                $out[] = '';
                $out[] = '```json';
                $out[] = trim( $schema );
                $out[] = '```';
